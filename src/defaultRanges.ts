...
export const defaultInputRanges = [
  {
    ...
    range(value: number | string) {
      ...
      const normalizedValue = Number(value) || 0; // Treat empty value as 0
      return {
        startDate: addDays(defineds.startOfToday, (Math.max(normalizedValue, 1) - 1) * -1),
        endDate: defineds.endOfToday
      };
    },
    getCurrentValue(range: DateRange) {
  ...
  },
  {
    ...
    range(value: number | string) {
      ...
      return {
        startDate: today,
        endDate: addDays(today, Math.max(normalizedValue, 1) - 1)
      };
    },
    getCurrentValue(range: DateRange) {
      if (!range.startDate || !range.endDate) return '';
